...
void UOArt::loadUOP(ClientFileType fileType, const QDateTime &lastModified, const std::string& uopPath)
{
    m_lastFileType = fileType;
    m_uopLastModified = lastModified;
    m_uopPackage.reset(new uopp::UOPPackage());
    m_uopPackage->load(uopPath);
}
...
QImage* UOArt::drawArtEnhanced(bool drawLegacy, unsigned int id, unsigned int hueIndex, bool partialHue)
{
    ...
    if (!drawLegacy)
    {
        ...
        if ((m_lastFileType != ClientFileType::TextureUOP) || (lastModified != m_uopLastModified))
            loadUOP(ClientFileType::TextureUOP, lastModified, uopPath_EnhancedTexture);
    ...
    if (drawLegacy)
    {
        ...
        if ((m_lastFileType != ClientFileType::LegacyTextureUOP) || (lastModified != m_uopLastModified))
            loadUOP(ClientFileType::LegacyTextureUOP, lastModified, uopPath_LegacyTexture);
    ...
    if (!uopFile->readData(fin, &uopErrorQueue) || !uopFile->unpack(decompressedDataVec, &uopErrorQueue))
    {
...
bool UOArt::getClassicPixelData(bool drawFromUOP, unsigned int id, std::vector<char> *data)
{
    if (drawFromUOP)
    {
        ...
        if (lastModified != m_uopLastModified)
            loadUOP(ClientFileType::TextureUOP, lastModified, CC_UOPPath);
